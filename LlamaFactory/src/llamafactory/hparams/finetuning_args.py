...
class FinetuningArguments(
    ...
    use_head_gate: bool = field(
        default=False,
        metadata={"help": "Whether or not to enable runtime head-gate patch for attention outputs."},
    )
    gate_hidden_dim: int | None = field(
        default=None,
        metadata={"help": "Hidden size of each head gate MLP. Defaults to attention head_dim."},
    )
    gate_bias_init: float = field(
        default=0.0,
        metadata={"help": "Initial bias of the last gate projection layer."},
    )
    gate_init_noise_std: float = field(
        default=1e-3,
        metadata={"help": "Std of Gaussian noise used to initialize gate projection weights."},
    )
    gate_train_only: bool = field(
        default=False,
        metadata={"help": "Whether or not to freeze all params except head gate MLPs."},
    )
    gate_reg_lambda: float = field(
        default=0.0,
        metadata={"help": "Regularization coefficient of (gate_mean - gate_target_ratio)^2."},
    )
    gate_target_ratio: float = field(
        default=0.2,
        metadata={"help": "Target average gate value for gate regularization."},
    )
    ...
    def __post_init__(self):
        ...
        if self.gate_train_only and not self.use_head_gate:
            raise ValueError("`gate_train_only` requires `use_head_gate=True`.")
