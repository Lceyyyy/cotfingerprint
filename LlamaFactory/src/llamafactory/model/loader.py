...
from .model_utils.head_gate import apply_head_gate_patch, setup_gate_trainable_params
...
def load_model(
    ...
    if not lazy_load:
        ...
        if finetuning_args.use_head_gate:
            patched_modules = apply_head_gate_patch(model, finetuning_args)
            logger.info_rank0(f"Enabled head-gate patch on {patched_modules} attention modules.")
        register_autoclass(config, model, tokenizer)

    model = init_adapter(config, model, model_args, finetuning_args, is_trainable)
    if is_trainable and finetuning_args.use_head_gate and finetuning_args.gate_train_only:
        trainable_gate_params = setup_gate_trainable_params(model)
        logger.info_rank0(f"Head-gate only training enabled. trainable gate params: {trainable_gate_params:,}")
