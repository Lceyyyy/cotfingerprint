...
from ...model.model_utils.head_gate import compute_gate_mean
...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, *args, **kwargs):
        ...
        if self.finetuning_args.use_asft_loss:
            ...
            super_outputs = super().compute_loss(model, inputs, *args, **kwargs)
            if isinstance(super_outputs, tuple):
                loss, outputs = super_outputs
            else:
                loss = super_outputs
            lm_loss = loss

        if self.finetuning_args.use_head_gate and self.finetuning_args.gate_reg_lambda > 0:
            gate_mean = compute_gate_mean(model)
            if gate_mean is not None:
                gate_target = torch.tensor(
                    self.finetuning_args.gate_target_ratio,
                    device=gate_mean.device,
                    dtype=gate_mean.dtype,
                )
                reg_loss = self.finetuning_args.gate_reg_lambda * (gate_mean - gate_target).pow(2)
                loss = loss + reg_loss

                if self.state.global_step % max(1, self.args.logging_steps) == 0:
                    self.log(
                        {
                            "transformer_lm_loss": float(lm_loss.detach().item()),
                            "gate_mean": float(gate_mean.detach().item()),
                            "gate_reg_loss": float(reg_loss.detach().item()),
                            "gate_total_loss": float(loss.detach().item()),
                        }
                    )
